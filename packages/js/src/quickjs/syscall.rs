...
pub fn syscall<'js>(
	...
	let func: qjs::Function = match name.as_str() {
		...
		"host_stop_close" => qjs::Function::new(ctx.clone(), Async(self::host::stop_close)),
		"host_stop_open" => qjs::Function::new(ctx.clone(), Async(self::host::stop_open)),
		"host_stop_stop" => qjs::Function::new(ctx.clone(), Async(self::host::stop_stop)),
