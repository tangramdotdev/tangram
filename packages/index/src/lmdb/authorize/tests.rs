...
fn sandbox_process_facts_preserve_all_relationships() {
	...
	for (process, sandbox) in [(&moved, &other), (&current, &sandbox)] {
		...
		let key = Key::Process(crate::lmdb::process::Key::Process(process.clone()));
		put(&index, &mut txn, &key, &data.serialize().unwrap());
	}
	txn.commit().unwrap();
	...
	let mut ids = Vec::new();
	loop {
		let request = crate::authorize::facts::Request::SandboxProcesses {
			after,
			limit: 1,
			sandbox: sandbox.clone(),
		};
		let output = Index::execute_authorization_fact_with_transaction(
			&index.db,
			&index.subspace,
			&txn,
			&request,
		...
		.unwrap();
		let crate::authorize::facts::Output::Ids {
			after: next,
			ids: page,
		} = output
		else {
