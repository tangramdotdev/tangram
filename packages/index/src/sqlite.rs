...
pub(super) fn indexer_put_cache_entries(
...
pub(super) fn indexer_put_objects(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
...
pub(super) fn indexer_touch_objects(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
...
pub(super) fn indexer_put_processes(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
	...
	for message in messages {
		...
		if !inserted {
			...
			let mut rows = query_statement
				.query(params)
				.map_err(|source| tg::error!(!source, "failed to execute the query statement"))?;
			let old_row = rows
				.next()
			...
			let mut rows = update_statement
				.query(params)
				.map_err(|source| tg::error!(!source, "failed to execute the update statement"))?;
			let row = rows
				.next()
				.map_err(|source| tg::error!(!source, "failed to get next row"))?
				.ok_or_else(|| tg::error!("expected a row"))?;
...
pub(super) fn indexer_touch_processes(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
...
pub(super) fn indexer_put_tags(
	...
	for message in messages {
		...
		let old_item = rows
			.next()
			.map_err(|source| tg::error!(!source, "failed to execute the statement"))?
			.map(|row| <Row as db::sqlite::row::Deserialize>::deserialize(row).map(|row| row.item))
			.transpose()
...
pub(super) fn indexer_delete_tags(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
...
pub(super) fn indexer_increment_transaction_id(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
) -> tg::Result<()> {
	let statement = indoc!(
...
pub(super) fn indexer_handle_stored_object(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
	n: usize,
) -> tg::Result<usize> {
	...
	let items = statement
		...
		.map(|result| result.map_err(|source| tg::error!(!source, "failed to deserialize the row")))
	...
	let mut enqueue_parents_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the enqueue parents statement"))?;
...
pub(super) fn indexer_handle_stored_process(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
	n: usize,
) -> tg::Result<usize> {
	...
	let mut update_subtree_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
	...
	let mut update_node_command_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
	...
	let mut update_node_log_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
	...
	let mut update_node_output_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
	...
	let mut update_node_error_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
	...
	let mut update_subtree_command_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
	...
	let mut update_subtree_log_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
	...
	let mut update_subtree_output_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
	...
	let mut update_subtree_error_stored_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the update stored statement"))?;
...
pub(super) fn indexer_handle_reference_count_cache_entry(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
	n: usize,
) -> tg::Result<usize> {
	...
	let mut reference_count_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the refence count statement"))?;
...
pub(super) fn indexer_handle_reference_count_object(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
	n: usize,
) -> tg::Result<usize> {
	...
	let mut reference_count_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the reference count statement"))?;
	...
	let mut cache_entries_statement = cache
		.get(transaction, statement.into())
		.map_err(|source| tg::error!(!source, "failed to prepare the cache entries statement"))?;
	...
	for item in &items {
		...
		children_statement
			.execute(params)
			.map_err(|source| tg::error!(!source, "failed to execute the children statement"))?;
...
pub(super) fn indexer_handle_reference_count_process(
	transaction: &sqlite::Transaction<'_>,
	cache: &db::sqlite::Cache,
	n: usize,
) -> tg::Result<usize> {
