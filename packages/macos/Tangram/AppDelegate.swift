...
private final class VfsBroker: @unchecked Sendable {
	...
	func stop() {
		let (listenSocket, mountedPaths) = withState {
			generation &+= 1
			let listenSocket = self.listenSocket
			self.listenSocket = -1
			let mountedPaths = self.mountedPaths
			self.mountedPaths.removeAll()
			return (listenSocket, mountedPaths)
		}
		if listenSocket >= 0 {
	...
	private func acceptLoop(_ listenSocket: Int32, generation: UInt64) {
		while isActive(generation) {
			...
			handleConnection(fd, generation: generation)
			Darwin.close(fd)
		}
	}

	private func handleConnection(_ fd: Int32, generation: UInt64) {
		do {
			let data = try readLine(from: fd)
			let request = try JSONDecoder().decode(VfsBrokerRequest.self, from: data)
			try handle(request, generation: generation)
	...
	private func handle(_ request: VfsBrokerRequest, generation: UInt64) throws {
	...
	private func mountWithApproval(
		dataDirectory: String,
		generation: UInt64,
		at mountPath: String,
		cache: VfsBrokerCache?,
		principal: String?,
		socket: String,
	) throws {
	...
	private func mountOptions(
		...
		if let cache {
			...
			if !cache.path.contains(",") {
				...
				logger.error("the cache path contains a comma, so it cannot be sent to the file system extension")
			}
			if let posixSemPrefix = cache.posixSemPrefix {
				if !posixSemPrefix.contains(",") {
					options.append("cache_posix_sem_prefix=\(posixSemPrefix)")
				} else {
					logger.error("the cache semaphore prefix contains a comma, so it cannot be sent to the file system extension")
				}
			}
		}
	...
	private func mountOnce(
		...
		let state = withState {
			(active: generation == self.generation, mounted: mountedPaths.contains(path))
		}
		guard state.active else {
			throw VfsBrokerError.mountFailed("the VFS broker stopped")
		}
		guard !state.mounted else {
			return (0, "")
		}
		...
		let arguments = [
			"-F",
			"-t", "tangram",
			"-o", mountOptions(cache, principal: principal, socket: socket),
			URL(filePath: dataDirectory, directoryHint: .isDirectory).standardizedFileURL.path(),
			path,
		]

		logger.info("mounting: /sbin/mount \(arguments.joined(separator: " "), privacy: .public)")
	...
	private func isActive(_ generation: UInt64) -> Bool {
		withState { generation == self.generation }
	}
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSToolbarDelegate {
	...
	private func startServer() {
		...
		do {
			...
			// Ensure the shared app group container exists so the server can bind its
			// socket there for the sandboxed file system extension to connect to.
			if let groupContainer = FileManager.default.containerURL(
				...
				appendDiagnostic("app group container: \(groupContainer.path())")
			} else {
				appendDiagnostic("failed to resolve the app group container")
			}
			for directory in [
			...
			// Pass the app group identifier so the server opens the cache's
