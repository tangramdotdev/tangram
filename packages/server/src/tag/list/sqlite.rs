	...
	pub(in crate::tag) fn match_tags_sqlite_sync(
		...
		for pattern_component in pattern.components() {
			...
			for m in matches {
				if pattern_component == "*" {
				...
				} else if pattern_component.contains(['=', '>', '<', '^']) {
					let statement = indoc!(
						"
							select id, component, item
							from tags
						...
						.map_err(|source| tg::error!(!source, "failed to get the next row"))?
					{
						...
						let item = row
							...
							.transpose()
							.map_err(|source| tg::error!(!source, "failed to parse the item"))?;
						if tg::tag::pattern::matches(&component, pattern_component) {
							let mut tag = m.as_ref().map_or_else(tg::Tag::empty, |m| m.tag.clone());
							tag.push(&component);
							let m = Match { id, tag, item };
							new.push(Some(m));
					...
					let mut statement = transaction
						...
						.map_err(|source| tg::error!(!source, "failed to prepare the statement"))?;
					let params =
						sqlite::params![m.as_ref().map_or(0, |m| m.id), pattern_component.to_string()];
					let mut rows = statement
						.query(params)
						.map_err(|source| tg::error!(!source, "failed to execute the statement"))?;
					while let Some(row) = rows
