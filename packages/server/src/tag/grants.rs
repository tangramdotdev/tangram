...
impl Session {
	pub(crate) async fn try_get_tag_grants(
		&self,
		tag: &tg::tag::Selector,
		...
		match location {
			tg::Location::Local(_) => self.try_get_tag_grants_local(tag).await,
			tg::Location::Remote(remote) => self.try_get_tag_grants_remote(tag, arg, remote).await,
		}
	}

	async fn try_get_tag_grants_local(
		&self,
		tag: &tg::tag::Selector,
	) -> tg::Result<Option<tg::tag::grants::Output>> {
		...
		Ok(Some(tg::tag::grants::Output { data }))
	}

	async fn try_get_tag_grants_remote(
		&self,
		tag: &tg::tag::Selector,
		mut arg: tg::tag::grants::Arg,
		...
		client.try_get_tag_grants(tag, arg).await.map_err(
			|error| tg::error!(!error, remote = %remote.name, "failed to list the tag grants"),
		)
	}

	pub(crate) async fn try_get_tag_grants_request(
		&self,
