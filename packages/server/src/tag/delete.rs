...
impl Server {
	pub async fn delete_tag(&self, mut arg: tg::tag::delete::Arg) -> tg::Result<tg::tag::delete::Output> {
		...
		let output = match &self.database {
			...
			Database::Postgres(database) => {
				Self::delete_tag_postgres(database, &arg.pattern).await?
			},
			Database::Sqlite(database) => {
				Self::delete_tag_sqlite(database, &arg.pattern).await?
			},
		};

		// Send a delete tag index message for each leaf tag only.
		for tag in &output.leaf_deleted {
