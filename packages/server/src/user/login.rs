...
impl Session {
	...
	async fn login_user_local(
		...
		crate::database::run!(&self.server.database, |transaction| {
				...
				let namespace_id =
						...
						.await?
					{
						...
						ControlFlow::Continue(error) => {
							return Ok::<
								ControlFlow<tg::user::login::Output, crate::database::Error>,
								tg::Error,
							>(ControlFlow::Continue(error));
						},
					};
			...
			if let Some(email) = email.clone() {
				...
				let result = transaction
					.query_optional_value_into::<String>(
						statement.into(),
						db::params![email.clone()],
					)
					.await;
			...
			Ok::<ControlFlow<tg::user::login::Output, crate::database::Error>, tg::Error>(
	...
	async fn login_user_remote(
		...
		crate::database::run!(&self.server.database, |transaction| {
			...
			let params = db::params![remote_name.clone(), token.clone(), user.clone()];
			let result = transaction.execute(statement.into(), params).await;
