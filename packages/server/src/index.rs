...
impl Server {
	...
	pub(crate) async fn indexer_task(&self, config: &crate::config::Indexer) -> tg::Result<()> {
		...
		loop {
			...
			let messages = match result {
				...
				Ok(None) => {
					dbg!("the stream ended");
					panic!("the stream ended")
				},
				Err(error) => {
					dbg!("failed to get a batch of messages");
	...
	async fn indexer_task_handle_messages(
		...
		for messages in batches {
			dbg!(&messages.iter().map(|(m, _)| m).collect::<Vec<_>>());
			...
			match &self.index {
				Either::Left(index) => {
					self.indexer_insert_objects_sqlite(object_messages, index)
						.await?;
					dbg!(&process_messages.iter().map(|(m, _)| m).collect::<Vec<_>>());
