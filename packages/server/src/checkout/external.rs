use {
	crate::Session,
	futures::{FutureExt as _, Stream, TryStreamExt as _},
...
impl Session {
	...
	> {
		...
		*path = self.host_path_for_guest_path(path)?;
		...
		let task = Task::spawn({
			...
			move |_| async move {
				// Ensure the artifact is available.
				let result = session
					.checkout_ensure_available(&artifact, &progress)
					.await
					.map_err(
						|error| tg::error!(!error, artifact = %artifact_id, "failed to ensure the artifact is available"),
					);
					...
					async move {
						if let tg::progress::Event::Output(mut output) = event {
							for path in &mut output.paths {
								*path = session.host_path_for_guest_path(path)?;
							}
	...
	pub(crate) async fn checkout_ensure_available(
		&self,
		artifact: &tg::Referent<tg::artifact::Id>,
		progress: &crate::progress::Handle<tg::checkout::Output>,
	) -> tg::Result<()> {
	...
	async fn checkout_task(
		...
		let output = tg::checkout::Output { paths: vec![path] };
	...
	fn checkout_dependency(&self, state: &mut State, item: &Item) -> tg::Result<()> {
		...
		let store_path = state
			.store_path
			.as_ref()
			.ok_or_else(|| tg::error!("cannot check out a dependency without a store path"))?;
		...
		let target = if let Some(mut edge) = node.artifact.clone() {
			...
			if dependency_item.id == state.artifact {
				...
				let store_path = state
					.store_path
					.as_ref()
					.ok_or_else(|| tg::error!("expected there to be a store path"))?;
