...
impl Server {
	...
	pub(crate) async fn document_with_context(
		...
		let output = match location {
			crate::location::Location::Local { region: None } => self.document_local(arg).await?,
			crate::location::Location::Local {
				region: Some(region),
			} => self.document_region(arg, region).await?,
			crate::location::Location::Remote { remote, region } => {
				self.document_remote(arg, remote, region).await?
			},
		};
