...
impl Server {
	...
	pub fn write_guard(&self) -> impl Future<Output = tokio::sync::RwLockWriteGuard<'_, ()>> {
		self.rw_lock.write()
	}

	pub fn read_guard(&self) -> impl Future<Output = tokio::sync::RwLockReadGuard<'_, ()>> {
		self.rw_lock.read()
	}
}
