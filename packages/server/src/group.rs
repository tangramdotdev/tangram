...
impl Session {
	...
	pub(crate) async fn grant_group_namespace_permission(
		&self,
		group: &str,
		...
		let grant = Self::grant_namespace_to_group_with_transaction(
			&transaction,
			&arg.namespace,
			namespace_id,
			&group.id,
	...
	pub(crate) async fn grant_group_namespace_permission_request(
		&self,
		request: http::Request<BoxBody>,
		group: &str,
	) -> tg::Result<http::Response<BoxBody>> {
		...
		let output = self
			...
			.await
			.map_err(
				|error| tg::error!(!error, %group, "failed to grant the namespace permission"),
			)?;
		let (content_type, body) = match accept
			.as_ref()
			.map(|accept| (accept.type_(), accept.subtype()))
