...
impl Server {
					...
					match message {
						...
						tg::sync::Message::Complete(tg::sync::CompleteMessage::Process(
							...
							let complete = crate::process::complete::Output {
								...
								commands: message.commands_complete,
								output: message.output_complete,
								outputs: message.outputs_complete,
							};
	...
	async fn sync_put_inner_process_batch(
		...
		while i < items.len() {
			...
			let is_complete = if state.arg.recursive {
				process_complete.children
					&& (!state.arg.commands || process_complete.commands)
					&& (!state.arg.outputs || process_complete.outputs)
			} else {
		...
		for ((item, complete), output) in std::iter::zip(items_with_complete, outputs) {
			...
			if state.arg.recursive && !complete.as_ref().map_or(false, |c| c.children) {
				state
					.queue_counter
			...
			if state.arg.commands {
				let command_complete = if state.arg.recursive {
					complete.as_ref().is_some_and(|c| c.commands)
				} else {
			...
			if state.arg.outputs {
				let output_complete = if state.arg.recursive {
					complete.as_ref().is_some_and(|c| c.outputs)
				} else {
