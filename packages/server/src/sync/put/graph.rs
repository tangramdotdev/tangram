			...
			match complete {
				Either::Left(process_complete) => {
					if let Some(NodeInner::Process(process_node)) = &mut node.inner {
						...
						process_node.complete.commands |= process_complete.commands;
						process_node.complete.output |= process_complete.output;
						process_node.complete.outputs |= process_complete.outputs;
					} else {
				...
				Some(NodeInner::Process(process)) => {
					if self.arg.recursive {
						process.complete.children
							&& (!self.arg.commands || process.complete.commands)
							&& (!self.arg.outputs || process.complete.outputs)
					} else {
					...
					Some(NodeInner::Process(process)) => {
						if self.arg.recursive {
							...
							if self.arg.commands {
								process.complete.commands = true;
							}
							if self.arg.outputs {
								process.complete.outputs = true;
							}
						} else {
							if self.arg.commands {
