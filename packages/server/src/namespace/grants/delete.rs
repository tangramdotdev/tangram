...
impl Session {
	pub(crate) async fn delete_namespace_grant(
		&self,
		...
		match location {
			tg::Location::Local(_) => self.delete_namespace_grant_local(arg).await,
			tg::Location::Remote(remote) => self.delete_namespace_grant_remote(arg, remote).await,
		}
	}

	async fn delete_namespace_grant_local(
		&self,
		arg: tg::namespace::grants::delete::Arg,
	) -> tg::Result<Option<()>> {
		...
		self.server
			...
			.run(|transaction| {
				let arg = arg.clone();
				async move {
					Self::delete_namespace_grant_inner_with_transaction(transaction, &arg).await
				}
				.boxed()
			})
			.await
	...
	async fn delete_namespace_grant_inner_with_transaction(
	...
	async fn delete_namespace_grant_remote(
		&self,
		mut arg: tg::namespace::grants::delete::Arg,
		...
		client.delete_namespace_grant(arg).await.map_err(|error| {
			tg::error!(
				!error,
				remote = %remote.name,
				"failed to delete the namespace grant"
			)
		})
	}

	pub(crate) async fn delete_namespace_grant_request(
		&self,
