...
impl Session {
	pub(crate) async fn try_get_process_tty_size_stream(
		...
		if let Some(local) = &locations.local {
			if local.current
				&& let Some(stream) = self
					...
					.await
					.map_err(|error| tg::error!(!error, "failed to get the process tty stream"))?
			{
		...
		if let Some(stream) = self
			...
			.await
			.map_err(|error| {
				tg::error!(!error, "failed to get the process tty stream from a remote")
			})? {
			return Ok(Some(stream));
		}
	...
	async fn try_get_process_tty_size_stream_local(
		...
		let stream = self
			...
			.map(|result| {
				let message =
					result.map_err(|error| tg::error!(!error, "failed to get message"))?;
	...
	async fn try_get_process_tty_size_stream_region(
		...
		let Some(stream) = client
			...
			.await
			.map_err(
				|error| tg::error!(!error, region = %region, "failed to get the process tty stream"),
			)?
		else {
			return Ok(None);
		};
	...
	async fn try_get_process_tty_size_stream_remote(
		...
		let Some(stream) = client
			...
			.await
			.map_err(
				|error| tg::error!(!error, remote = %remote.name, "failed to get the process tty stream"),
			)?
		else {
			return Ok(None);
		};
