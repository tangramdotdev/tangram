...
impl Server {
	pub(crate) async fn try_set_process_pty_size_with_context(
		&self,
		_context: &Context,
		id: &tg::process::Id,
		...
		if let Some(remote) = Self::remote(arg.local, arg.remotes.as_ref())? {
			...
			return client.set_process_pty_size(id, arg).await;
		}
	...
	pub(crate) async fn handle_set_process_pty_size_request(
		&self,
		...
		// Put the process pty.
		self.try_set_process_pty_size_with_context(context, &id, arg)
			.await
