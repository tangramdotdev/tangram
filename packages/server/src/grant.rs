...
impl Session {
	...
	// Grant the public principal the process read permissions it is missing, idempotently. Safe on a fresh process or a cache hit.
	pub(crate) async fn make_process_public(&self, id: &tg::process::Id) -> tg::Result<()> {
		let resource = tg::Id::from(id.clone());

		// Read the permissions the public principal already has on the process.
		let existing =
			{
				let mut connection =
		...
		// Grant only the missing read permissions; create_grant authorizes the builder holds them and will not duplicate.
