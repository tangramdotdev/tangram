...
> {
	...
	if bytes
		...
		== [0x50, 0x4b]
	{
		return Ok(Some((tg::artifact::archive::Format::Zip, None)));
	}
	...
	if bytes
		...
		== b"ustar\0"
	{
		return Ok(Some((tg::artifact::archive::Format::Tar, None)));
	}
...
pub fn detect_compression_format(bytes: &[u8]) -> tg::Result<Option<tg::blob::compress::Format>> {
	// Gz
	let n = bytes
		.get(..2)
		.ok_or_else(|| tg::error!("buffer is too small"))?;
	if n == [0x1F, 0x8B] {
		return Ok(Some(tg::blob::compress::Format::Gz));
	}
	...
	if n == [0x28, 0xB5, 0x2F, 0xFD] {
		return Ok(Some(tg::blob::compress::Format::Zstd));
	}
	...
	let n = bytes
		...
		.ok_or_else(|| tg::error!("buffer is too small"))?;
	if n == [0x42, 0x5A, 0x68] {
		return Ok(Some(tg::blob::compress::Format::Bz2));
	}
	...
	if n == [0xFD, 0x37, 0x7A, 0x58, 0x5A, 0x00] {
		return Ok(Some(tg::blob::compress::Format::Xz));
	}
