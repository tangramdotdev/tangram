...
impl Server {
	pub(crate) async fn indexer_task(&self, config: &crate::config::Indexer) -> tg::Result<()> {
		...
		let object_cache_future = async move {
			object_cache_task
				.wait()
				.await
				.map_err(|error| tg::error!(!error, "the object cache task panicked"))?
		};
		...
		future::try_join_all([
			database_index_outbox_future.boxed(),
			grant_update_future.boxed(),
			log_compaction_future.boxed(),
			node_update_future.boxed(),
			object_archive_outbox_future.boxed(),
			object_cache_future.boxed(),
			object_index_outbox_future.boxed(),
			request_future.boxed(),
			storage_update_future.boxed(),
			usage_aggregation_future.boxed(),
		])
		.await?;
