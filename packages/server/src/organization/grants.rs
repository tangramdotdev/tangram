...
impl Session {
	pub(crate) async fn try_get_organization_grants(
		...
		match location {
			tg::Location::Local(_) => self.try_get_organization_grants_local(organization).await,
			tg::Location::Remote(remote) => {
				self.try_get_organization_grants_remote(organization, arg, remote)
					.await
			},
		}
	}

	async fn try_get_organization_grants_local(
		&self,
		organization: &tg::organization::Selector,
	) -> tg::Result<Option<tg::organization::grants::Output>> {
		let authorized = self
	...
	async fn try_get_organization_grants_remote(
		&self,
		organization: &tg::organization::Selector,
		mut arg: tg::organization::grants::Arg,
		...
		client
			...
			.await
			.map_err(
				|error| tg::error!(!error, remote = %remote.name, "failed to list the organization grants"),
			)
	}
