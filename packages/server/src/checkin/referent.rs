...
impl Session {
	pub(super) async fn try_checkin_artifact_path(
		&self,
		path: &Path,
	...
	async fn checkin_artifact_path(
		&self,
		id: tg::artifact::Id,
		path: &Path,
		...
		let sandbox = self.try_get_checkin_sandbox()?;
	...
	fn try_get_checkin_sandbox(&self) -> tg::Result<Option<tg::sandbox::Id>> {
