...
impl Server {
	...
	fn checkin_visit(&self, state: &mut State, path: PathBuf) -> tg::Result<Option<usize>> {
		...
		let lock_node = state
			.lock_paths
			.as_ref()
			.and_then(|lock_paths| lock_paths.get(&path).copied());
		...
		let dependencies =
			if let Ok(Some(contents)) = xattr::get(&path, tg::file::DEPENDENCIES_XATTR_NAME) {
				...
				for reference in references {
					...
					{
						let reference = path.parent().unwrap().join(reference_path);
						let reference = reference.canonicalize().map_err(
							|source| tg::error!(!source, %path = reference.display(), "failed to canonicalize the path"),
						)?;
						let Some(index) = self.checkin_visit(state, reference.clone())? else {
							continue;
						};
