...
impl Server {
	pub(super) async fn checkin_resolve_subpaths(
		...
		let old_graph = graph.clone(); // Clone of the graph before fixups.
					...
					let edge = if let Some(graph) = &pointer.graph {
						...
						match entry {
							tg::graph::Edge::Object(object) => {
								tg::graph::data::Edge::Object(object.id())
							},
					...
					match edge {
						tg::graph::data::Edge::Object(id) => {
							if components.peek().is_none() {
								return Ok(Some(tg::graph::data::Edge::Object(id.clone().into())));
							}
							let directory = id.clone().into();
