	...
	async fn resolve_reference(
		...
		if current.objects.is_none() {
			...
			let objects: im::Vector<_> = self
				...
				.into_iter()
				.filter_map(|output| {
					eprintln!("{} -> {}", output.tag, output.item);
...
impl Server {
	fn fix_unification_subpaths(state: &mut super::State) {
		for index in 0..state.graph.nodes.len() {
			...
			let mut dependencies = state.graph.nodes[index].variant.unwrap_file_ref().dependencies.clone();
			for dependency in &mut dependencies  {
				match dependency {
					FileDependency::Import { subpath, node: Some(node), .. } if subpath.is_none() => {
			...
			state.graph.nodes[index].variant.unwrap_file_mut().dependencies = dependencies;
		}
	}
}
...
fn try_backtrack(state: &mut Vec<State>, edge: &Unresolved) -> Option<State> {
	eprintln!("backtracking");
	// Go back to where the reference was originally solved.
	let package = edge.reference.name()?;
	let position = state
		.iter()
		.position(|state| {
			let keys = state.packages.keys().collect::<Vec<_>>();
			eprintln!("packages: {keys:#?}");
