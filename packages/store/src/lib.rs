...
pub trait Store {
	...
	fn delete_outbox_fragments(
		&self,
		arg: outbox::fragment::delete::Arg,
	) -> impl std::future::Future<Output = tangram_client::Result<()>> + Send;

	fn dequeue_outbox_fragments(
		&self,
		arg: outbox::fragment::dequeue::Arg,
	) -> impl std::future::Future<Output = tangram_client::Result<Vec<outbox::fragment::Fragment>>> + Send;

	fn enqueue_outbox_batch(
		&self,
		arg: outbox::batch::enqueue::Arg,
	) -> impl std::future::Future<Output = tangram_client::Result<()>> + Send;
	...
	) -> impl std::future::Future<Output = tangram_client::Result<Option<outbox::batch::Id>>> + Send;
