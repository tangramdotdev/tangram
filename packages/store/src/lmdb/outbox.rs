...
impl Store {
	...
	pub async fn enqueue_outbox_batch(&self, arg: batch::enqueue::Arg) -> tg::Result<()> {
		let request = super::request::Request::EnqueueOutboxBatch(arg);
	...
	pub(super) fn dequeue_outbox_fragments_with_transaction(
		...
		for partition in arg.partition_start..arg.partition_end {
			let prefix = fdbt::pack(&(Kind::OutboxFragment.to_i32().unwrap(), partition));
			let entries = db
	...
	pub(super) fn delete_outbox_fragments_with_transaction(
		...
		for fragment in arg.fragments {
			let key = StoreKey::Outbox(Key::Fragment {
...
mod tests {
	...
	async fn operations_and_persistence() {
		...
		let fragments = fragments
			...
			.collect();
		store
			.delete_outbox_fragments(fragment::delete::Arg { fragments })
			.await
			.unwrap();
		assert!(
			store
