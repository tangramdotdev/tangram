...
impl Store {
	pub fn delete_outbox_fragments(&self, arg: fragment::delete::Arg) {
	...
	pub fn dequeue_outbox_fragments(
		&self,
	...
	pub fn enqueue_outbox_batch(&self, arg: batch::enqueue::Arg) -> tg::Result<()> {
		let mut state = self.state();
...
mod tests {
	...
	fn operations() {
		...
		store
			.enqueue_outbox_batch(batch::enqueue::Arg {
		...
		let fragments = store
			.dequeue_outbox_fragments(fragment::dequeue::Arg {
				batch_size: 1,
				partition_end: 1,
				partition_start: 0,
			})
		...
		let target = store
			.try_get_outbox_batch_at_or_before(batch::get::Arg {
				batch: None,
				partition_end: 2,
				partition_start: 0,
			})
		...
		let fragments = fragments
			...
			.collect();
		store.delete_outbox_fragments(fragment::delete::Arg { fragments });
		assert!(
			store
				.try_get_outbox_batch_at_or_before(batch::get::Arg {
					batch: Some(first),
