...
#[derive(Default)]
struct State {
	archive_queue: BTreeMap<(tg::indexer::Id, u64), crate::archive::queue::Entry>,
	index_queue: BTreeMap<(tg::indexer::Id, u64), crate::index::queue::Fragment>,
	logs: Logs,
...
mod tests {
	...
	fn put_replaces_object() {
		...
		let object = cache
			.try_get_object_sync(&object::get::Arg {
				bytes: true,
				id,
				put: None,
			})
	...
	async fn get_exact_put() {
		...
		let contains = crate::Cache::contains_object(
			&cache,
			object::contains::Arg {
				id: id.clone(),
				put: [1; 16],
		...
		let contains =
			crate::Cache::contains_object(&cache, object::contains::Arg { id, put: [2; 16] })
				.await
				.unwrap();
		assert!(contains);
	}
