use crate::{Enum, Field, Item, Struct, StructVariantField, Variant, VariantKind};

impl<'a> Item<'a> {
	pub fn parse(input: &'a syn::DeriveInput) -> syn::Result<Item<'a>> {
		let data = &input.data;
		let item = match data {
			syn::Data::Struct(data) => Item::Struct(Struct::parse(input, data)?),
			syn::Data::Enum(data) => Item::Enum(Enum::parse(input, data)?),
			syn::Data::Union(_) => {
