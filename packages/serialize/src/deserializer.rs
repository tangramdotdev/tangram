	...
	R: Read,
{
	...
	pub fn read_len(&mut self) -> Result<usize> {
		...
		let len = len.to_usize().ok_or_else(|| {
			Error::new(
				ErrorKind::Other,
				"The length is too long for this architecture.",
			)
		})?;
		Ok(len)
	}
	...
	pub fn read_uvarint(&mut self) -> Result<u64> {
		...
		if !success {
			return Err(Error::new(ErrorKind::Other, "Invalid varint."));
		}
		Ok(value)
	}
	...
	pub fn read_string(&mut self) -> Result<String> {
		...
		let value =
			String::from_utf8(value).map_err(|error| Error::new(ErrorKind::Other, error))?;
		Ok(value)
	}
