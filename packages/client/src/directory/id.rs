...
pub struct Id(tg::artifact::Id);

impl Id {
	#[must_use]
	pub fn new(bytes: &[u8]) -> Self {
		let id = tg::Id::new_blake3(tg::id::Kind::Directory, bytes);
		Self(tg::artifact::Id::try_from(id).unwrap())
	}
}

impl Deref for Id {
	type Target = tg::artifact::Id;
...
impl TryFrom<tg::Id> for Id {
	...
	fn try_from(value: tg::Id) -> tg::Result<Self, Self::Error> {
		if value.kind() != tg::id::Kind::Directory {
			return Err(tg::error!(%value, "expected a directory ID"));
		}
		Ok(Self(tg::artifact::Id::try_from(value)?))
	}
}
...
impl From<Id> for tg::artifact::Id {
	fn from(value: Id) -> Self {
		value.0
	}
}
