...
impl Directory {
		...
		H: tg::Handle,
	{
		match directory {
			...
			tg::graph::Directory::Branch(branch) => {
				// Traverse all children and collect their entries.
	...
	/// Resolve a directory edge to a Directory handle.
		...
		H: tg::Handle,
	{
		match directory {
			...
			tg::graph::Directory::Branch(branch) => {
				// Binary search to find the child containing this entry.
				// Children are ordered by their `last` key (lexicographically largest entry).
				// Find the first child where last >= name.
				...
				let Some(index) = child_index else {
					// Name is greater than all children's last keys, so it doesn't exist.
					return Ok(None);
				};
