	...
	Object + Process + Pipe + Pty + Remote + Tag + User + Send + Sync + 'static
{
	...
	) -> BoxFuture<'a, tg::Result<tg::blob::create::Output>>;
	...
	fn try_read_blob_stream<'a>(
		&'a self,
		id: &'a tg::blob::Id,
		arg: tg::blob::read::Arg,
	) -> BoxFuture<'a, tg::Result<Option<BoxStream<'static, tg::Result<tg::blob::read::Event>>>>>;
...
pub trait Pipe: Send + Sync + 'static {
	...
	) -> BoxFuture<'a, tg::Result<BoxStream<'static, tg::Result<tg::pipe::Event>>>>;
...
pub trait Pty: Send + Sync + 'static {
	...
	fn read_pty<'a>(
		...
		id: &'a tg::pty::Id,
		arg: tg::pty::read::Arg,
	) -> BoxFuture<'a, tg::Result<BoxStream<'static, tg::Result<tg::pty::Event>>>>;
