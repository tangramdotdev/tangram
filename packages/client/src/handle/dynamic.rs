...
impl tg::Handle for Handle {
	...
	fn create_blob(
		...
		self.0.create_blob(Box::pin(reader))
	}
	...
	> {
		unsafe {
			std::mem::transmute::<_, BoxFuture<'_, tg::Result<Option<BoxStream<_>>>>>(
				self.0.try_read_blob_stream(id, arg),
			)
		}
	}
...
impl tg::handle::Process for Handle {
	...
	) -> impl Future<Output = tg::Result<Option<tg::process::Metadata>>> + Send + Send {
		unsafe {
	...
	) -> impl Future<Output = tg::Result<Option<tg::process::get::Output>>> + Send + Send {
	...
	) -> impl Future<Output = tg::Result<()>> + Send + Send {
	...
	) -> impl Future<Output = tg::Result<tg::process::heartbeat::Output>> + Send + Send {
...
impl tg::handle::Pty for Handle {
	...
	fn get_pty_size(
		...
		id: &tg::pty::Id,
		arg: tg::pty::read::Arg,
	) -> impl Future<Output = tg::Result<Option<tg::pty::Size>>> + Send + Send {
...
impl tg::handle::Remote for Handle {
	...
	fn try_get_remote(
		&self,
		name: &str,
	) -> impl Future<Output = tg::Result<Option<tg::remote::get::Output>>> + Send + Send {
	...
	fn delete_remote(&self, name: &str) -> impl Future<Output = tg::Result<()>> + Send + Send {
...
impl tg::handle::Tag for Handle {
	...
	) -> impl Future<Output = tg::Result<Option<tg::tag::get::Output>>> + Send + Send {
...
impl tg::handle::User for Handle {
	fn get_user(
		&self,
		token: &str,
	) -> impl Future<Output = tg::Result<Option<tg::User>>> + Send + Send {
