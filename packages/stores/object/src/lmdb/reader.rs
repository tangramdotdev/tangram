...
impl Store {
	...
	fn execute_read_request(
		...
		let response = match request {
			crate::read::Request::DequeueOutboxFragments(arg) => {
				let output =
					Self::dequeue_outbox_fragments_with_transaction(db, transaction, &arg)?;
				crate::read::Response::DequeueOutboxFragments(output)
			},
			#[cfg(test)]
			crate::read::Request::GetTransactionId => {
				crate::read::Response::GetTransactionId(transaction.id() as u64)
			},
			crate::read::Request::TryGet(arg) => {
				let output = Self::try_get_object_with_transaction(db, transaction, &arg.id)?;
				let output = crate::TryGetOutput { object: output };
				crate::read::Response::TryGet(output)
			},
			crate::read::Request::TryGetBatch(arg) => {
				let output = Self::try_get_batch_with_transaction(db, transaction, &arg)?;
				crate::read::Response::TryGetBatch(output)
			},
			crate::read::Request::TryGetOutboxBatchAtOrBefore(arg) => {
				let output = Self::try_get_outbox_batch_at_or_before_with_transaction(
					db,
					transaction,
					&arg,
				)?;
				crate::read::Response::TryGetOutboxBatchAtOrBefore(output)
			},
		};
