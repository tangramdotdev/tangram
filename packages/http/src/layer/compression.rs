			...
			move |request: http::Request<BoxBody>| {
				...
				async move {
						...
						&& let Some((algorithm, level)) = (predicate)(&parts, &body)
					{
						...
						body = BoxBody::new(Compression::new(body, algorithm, level));
					}
			...
			move |request: http::Request<BoxBody>| {
				...
				async move {
					...
					if let Some(algorithm) = algorithm {
						body = BoxBody::new(Decompression::new(body, algorithm));
					}
					let request = http::Request::from_parts(parts, body);
					let response = service.ready().await?.call(request).await?;
					Ok(response)
	...
	S::Error: Send + 'static,
{
	...
	fn layer(&self, service: S) -> Self::Service {
		tower::util::BoxCloneSyncService::new(tower::service_fn(
			move |mut request: http::Request<BoxBody>| {
				let mut service = service.clone();
				async move {
