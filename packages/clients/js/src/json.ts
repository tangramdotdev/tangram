// The runtime model uses `undefined`; `null` is only a JSON wire token. These convert between the two, preserving keys so a present-but-empty map entry survives the round trip.
