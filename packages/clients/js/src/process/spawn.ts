...
let spawnArgFromResolvedWithSandbox = async (
	...
	if (command.state.token !== undefined) {
...
export let spawnSandboxed = async <O extends tg.Value = tg.Value>(
	...
	} else if (arg.tty != null && arg.tty !== false) {
		tty = arg.tty;
	}
	...
	let wait =
		output.wait != null ? tg.Process.Wait.fromData(output.wait) : undefined;
	let location =
	...
	let process = new tg.Process<O>({
		...
		...(stdioPromise !== undefined ? { stdioPromise } : {}),
		...(output.lease != null ? { lease: output.lease } : {}),
		stdout: new tg.Process.Stdio.Reader({
			unavailable: !provideStdout,
			stream: "stdout",
		}),
		...(output.token != null ? { token: output.token } : {}),
		...(wait !== undefined ? { wait } : {}),
	});
	return process;
};
...
function renderJsDebugArgs(debug?: tg.Process.Debug | null): Array<string> {
	...
	if (debug.mode != null && debug.mode !== "normal") {
...
let normalizeSandbox = (
	...
	let hasCpu = arg.cpu != null;
	let cpu = arg.cpu;
	let hasMemory = arg.memory != null;
	let memory = arg.memory;
	let mounts = arg.mounts ?? [];
	let hasNetwork = arg.network != null;
	let network = arg.network;
	let hasOwner = arg.owner != null;
	let owner = arg.owner;
	let ports = arg.ports ?? [];
	...
	if (isSandboxArg(sandbox)) {
		...
		if (sandbox.mounts !== undefined) {
			output.mounts =
				sandbox.mounts === null
					? null
					: sandbox.mounts.map(tg.Sandbox.Mount.toDataString);
		}
