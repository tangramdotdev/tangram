...
export class Mutation<T extends tg.Value = tg.Value> {
	...
	static toData<T extends tg.Value = tg.Value>(
		...
		if (value.inner.kind === "unset") {
			...
			if (
				value.inner.separator !== undefined &&
				value.inner.separator !== null
			) {
		...
		export let withoutLocationAndTokens = (
			...
			switch (data.kind) {
				...
				case "append": {
					return {
						...data,
						values: data.values.map(tg.Value.Data.withoutLocationAndTokens),
					};
				}
				case "prefix":
				case "suffix": {
					return {
						...data,
						template: tg.Template.Data.withoutLocationAndTokens(data.template),
					};
				}
				case "merge": {
					return {
						...
						value: globalThis.Object.fromEntries(
							globalThis.Object.entries(data.value).map(([key, value]) => [
								key,
								tg.Value.Data.withoutLocationAndTokens(value),
							]),
						),
					};
				}
