		...
		let nodes = await Promise.all(
			(arg.nodes ?? []).map(async (node) => {
				if (node.kind === "directory") {
					...
					let fileNode: tg.Graph.FileNode = {
						kind: "file" as const,
						contents,
						dependencies,
						executable,
					};
					if (node.module !== undefined && node.module !== null) {
						fileNode.module = node.module;
					}
					return fileNode;
				} else if (node.kind === "symlink") {
					let symlinkNode: tg.Graph.SymlinkNode = { kind: "symlink" as const };
					if (node.artifact !== undefined && node.artifact !== null) {
						symlinkNode.artifact = tg.Graph.Edge.fromArg(
							node.artifact,
							arg.nodes,
						);
					}
					if (node.path !== undefined && node.path !== null) {
						symlinkNode.path = node.path;
					}
					return symlinkNode;
				} else {
		...
		for (let arg of resolved) {
			...
			for (let argNode of argNodes) {
				if (argNode.kind === "directory") {
					...
					} else if (argNode.artifact != null && "index" in argNode.artifact) {
						artifact = {
							...argNode.artifact,
...
export namespace Graph {
	...
	export namespace Arg {
		...
		export type Directory = {
			entries?: { [name: string]: tg.Graph.Arg.Edge<tg.Artifact> } | null;
		};
	...
	export type Symlink = {
		artifact?: tg.Graph.Edge<tg.Artifact>;
		path?: string;
	};
	...
	export namespace Symlink {
		...
		export let fromData = (data: tg.Graph.Data.Symlink): tg.Graph.Symlink => {
			let symlink: tg.Graph.Symlink = {};
			if (data.artifact !== undefined) {
				symlink.artifact = tg.Graph.Edge.fromData(
					data.artifact,
					tg.Artifact.withId,
				);
			}
			if (data.path !== undefined) {
				symlink.path = data.path;
			}
			return symlink;
		};
	...
	export namespace Edge {
		export let fromArg = <T>(
			...
			if (typeof arg === "number") {
				...
				let pointer: tg.Graph.Pointer = { index: reference.index, kind };
				if (reference.graph !== undefined && reference.graph !== null) {
					pointer.graph = reference.graph;
				}
				return pointer;
			} else {
				return arg as T;
	...
	export namespace Pointer {
		...
		export let fromArg = (arg: tg.Graph.Arg.Pointer): tg.Graph.Pointer => {
			if (typeof arg === "number" || arg.kind == null) {
		...
		export let fromData = (data: tg.Graph.Data.Pointer): tg.Graph.Pointer => {
			if (typeof data === "string") {
				...
				if (data.graph !== undefined) {
					pointer.graph = tg.Graph.withId(data.graph);
				}
				return pointer;
			}
		};
		...
		export let fromDataString = (data: string): tg.Graph.Pointer => {
			...
			let object: tg.Graph.Pointer = { index, kind };
			if (graph !== undefined) {
				object.graph = graph;
			}
			return object;
	...
	export namespace Data {
		...
		export type DirectoryLeaf = {
			entries?: { [name: string]: tg.Graph.Data.Edge<tg.Artifact.Id> } | null;
		};
