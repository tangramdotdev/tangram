...
impl<O: 'static> tg::Process<O> {
		...
		Fut: Future<Output = tg::Result<tg::process::spawn::Output>>,
	{
		...
		let mut tty = match arg.tty.take() {
			Some(tg::Either::Left(true)) => tangram_util::tty::get_controlling_tty_size()
		...
		{
			tty = tangram_util::tty::get_controlling_tty_size()
				.map(|size| tg::process::tty::Size {
					cols: size.cols,
					rows: size.rows,
				})
		...
		H: tg::Handle,
	{
		...
		if exists {
			let output_bytes = tg::file::xattrs::read_sharded(&output_path, "user.tangram.output")
			...
			let error_bytes = tg::file::xattrs::read_sharded(&output_path, "user.tangram.error")
