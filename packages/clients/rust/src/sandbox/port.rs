...
impl Port {
	...
	pub fn expand(self) -> Vec<Self> {
		(0..self.container.len())
			.map(|index| {
				let host = self.host.map(|range| Range::single(range.port_at(index)));
				let container = Range::single(self.container.port_at(index));
				Self {
...
impl std::str::FromStr for Port {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		let (host_ip, host, container) = match parts.as_slice() {
			[container] => (None, None, container.parse::<Range>()?),
			[host, container] => {
				if host.is_empty() {
			...
			[host_ip, host, container] => {
				...
				(host_ip, host, container.parse::<Range>()?)
			},
			_ => return Err(tg::error!("invalid port mapping")),
		};
		if let Some(host) = host
			&& host.len() != container.len()
		{
			return Err(tg::error!(
				"host and container port ranges must have the same length"
			));
		}
		Ok(Self {
			host_ip,
