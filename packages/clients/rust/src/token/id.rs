...
impl std::ops::Deref for tg::token::Id {
	type Target = tg::Id;
...
impl From<tg::token::Id> for tg::Id {
	fn from(value: tg::token::Id) -> Self {
		value.0
	}
}

impl TryFrom<tg::Id> for tg::token::Id {
	type Error = tg::Error;

	fn try_from(value: tg::Id) -> tg::Result<Self, Self::Error> {
		if value.kind() != tg::id::Kind::Token {
...
impl TryFrom<Vec<u8>> for tg::token::Id {
	type Error = tg::Error;
...
impl std::str::FromStr for tg::token::Id {
	type Err = tg::Error;
...
impl TryFrom<String> for tg::token::Id {
	type Error = tg::Error;
...
mod tests {
	...
	fn roundtrip_string() {
		...
		assert_eq!(roundtrip, id);
		assert!(id.to_string().starts_with("tok_"));
	}
}
