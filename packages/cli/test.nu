...
export def --env spawn [
	...
	let ready_output = (open /dev/null |  bash -c 'od -An -t u1 -N1 "$1"' _ $ready_path | complete)
	rm -f $ready_path
	print $ready_output.stderr
	assert (($ready_output.stderr | str trim) == '') "the server had an error"
