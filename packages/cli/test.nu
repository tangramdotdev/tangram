...
def reset_database_instance_once [instance: string, pool_path: string] {
	...
	let results = ['foundationdb' 'postgres' 'scylla'] | par-each { |database|
		let output = match $database {
			...
			'scylla' => {
				(^timeout --kill-after 2s $database_reset_timeout_secs tangram_scylla_client 127.0.0.1 9042 -k $'store_($instance)' -e 'truncate indexers; truncate logs; truncate object_archive_queue; truncate object_cache; truncate object_index_queue; truncate objects;' | complete)
			},
		}
