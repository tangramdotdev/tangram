...
impl Cli {
	#[must_use]
	pub fn command_py(matches: &clap::ArgMatches, args: Args) -> std::process::ExitCode {
		match Self::command_py_inner(matches, args) {
			Ok(exit) => exit,
			Err(error) => {
