	...
	H: tg::Handle,
{
	pub fn new(handle: &H, item: Either<tg::Build, tg::Object>, expand_options: Options) -> Self {
		...
		let roots = vec![Node::new(None, provider, expand_options)];
	...
	pub fn stop(&self) {
		self.state
			...
			.roots
			.iter()
			.for_each(|root| root.read().unwrap().stop());
	}
		...
		let task = Task::spawn({
			...
			move |stop| async move {
				loop {
					...
					let stopped = pin::pin!(stop.stopped());
				...
				// - if the expand_options.collapse_builds_on_success is true
				...
				if !dependencies.is_empty() {
					let dependencies = dependencies
						...
						.map(|(reference, dependency)| {
							...
							(name, tg::Value::Object(dependency.object.clone()))
						})
						.collect();
