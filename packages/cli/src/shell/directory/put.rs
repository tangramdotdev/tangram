...
impl Cli {
	pub async fn command_shell_directory_put(&mut self, args: Args) -> tg::Result<()> {
		...
		let shell = config.shell.get_or_insert_default();
		let directory = crate::config::ShellDirectory {
			export: args.export,
		};
		shell.directories.insert(key, directory);
