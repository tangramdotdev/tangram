	...
	H: tg::Handle,
{
	...
	pub async fn run_fullscreen(
		...
		if !tangram_util::tty::is_foreground_controlling_tty(ttyfd) {
			return self.run_inline(stopper, true).await;
		}
	...
	pub async fn run_inline(&mut self, stopper: Stopper, print: bool) -> tg::Result<()> {
		...
		loop {
			...
			// If the tree has a process and stderr is a terminal, render the tree. Otherwise, clear guards.
			if self.tree.has_process()
				&& let Some(tty) = tty.as_mut()
			{
				// Render the tree.
				...
				let (columns, rows) = if let Some(size) = tangram_util::tty::get_tty_size(tty_fd) {
					(size.cols.to_usize().unwrap(), size.rows.to_usize().unwrap())
				} else {
