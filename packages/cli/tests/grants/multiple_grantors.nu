...
assert ($grants | any {|g| $g.subject == $bob.user.id and $g.permissions == { kind: group, value: [read] } }) "Alice's grant should be listed"
assert ($grants | any {|g| $g.subject == $bob.user.id and $g.permissions == { kind: group, value: [write] } }) "Carol's grant should be listed"
