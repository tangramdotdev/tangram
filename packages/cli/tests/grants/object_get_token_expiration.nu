...
let expiration = (token-body $first.tokens.local).expires_at
...
let job = job spawn {
	let job_id = job id
	let output = get-object $socket $bob.token $child --token $first.tokens.local
...
assert equal (token-body $delayed.tokens.local).expires_at $expiration
for child in ($delayed.children | values) {
	assert equal (token-body $child.tokens.local).expires_at $expiration
}
...
let refreshed = get-object $socket $alice.token $directory --token $first.tokens.local
let refreshed_expiration = (token-body $refreshed.tokens.local).expires_at
...
let child_token = $refreshed.children | get $child | get tokens.local
...
get-object $socket $bob.token $directory --token $refreshed.tokens.local | ignore
...
assert ((token-body $output.tokens.local).expires_at > $expiration)
assert ((token-body ($output.children | get $child | get tokens.local)).expires_at > $expiration)
