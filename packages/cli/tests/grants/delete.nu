...
assert (not ($grants | any {|g| $g.subject == $bob.user.id and $g.permissions == { kind: group, value: [read] } })) "the grant should be gone after delete"
...
let grants = tg --token $alice.token grants list --resource team | from json
assert (not ($grants | any {|g| $g.subject == $bob.user.id and $g.permissions == { kind: group, value: [write] } })) "revoke should remove the grant"
