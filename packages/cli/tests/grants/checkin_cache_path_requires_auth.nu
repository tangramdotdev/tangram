...
# Checking in a cache path must authorize the caller on the named artifact: the cache-path branch mints an object subtree authorization token for the artifact id in the path, so a principal who cannot read that artifact must not be able to check it in and obtain a read capability for it.
