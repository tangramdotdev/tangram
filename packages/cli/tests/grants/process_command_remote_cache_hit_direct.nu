...
let path = artifact {
	tangram.ts: '
		...
		export async function consume(directory: tg.Directory) {
		...
		export default async function () {
			...
			return tg.build(consume, directory);
		}
	',
}
...
assert equal $result.exit 0 "a process should be able to pass its cache-hit child's output directly to another child."
