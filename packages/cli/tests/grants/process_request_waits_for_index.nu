use ../../test.nu *

# A process starts before its index is ready, but its authenticated child spawn waits for the index.

let root_token = random chars

# Spawn the remote. It holds no runner role, so the build can only complete by way of the runner.
let remote = server spawn --preserve-keys --name remote --config {
	advanced: { single_process: false },
	authentication: { root: { token: $root_token }, users: { providers: { insecure: true } } },
	roles: [http indexer scheduler],
}

# Create the runner and its token.
let created = tg --url $remote.url --token $root_token runner create | from json

# Spawn the runner with checkpoints enabled.
let runner = server spawn --name runner --config {
	advanced: { checkpoints: true },
	remotes: { default: { token: $created.token.token, url: $remote.url } },
	roles: [http indexer runner],
	runner: { id: $created.runner.id, remote: 'default', token: $created.token.token },
}

# Create user credentials and spawn the local server.
let alice = tg --url $remote.url login --verbose --name alice | from json
let local = server spawn --name alice-local --config {
	remotes: { default: { token: $alice.token, url: $remote.url } },
}

# Hold the process index and observe the process start.
let index_watch = (
	tg --url $runner.url checkpoint watch runner.process.index.started
	| from json
	| get watch
)
let start_watch = (
	tg --url $runner.url checkpoint watch runner.process.start
	| from json
	| get watch
)

let path = artifact {
	tangram.ts: '
		export default () => {
			return tg.build(child);
		};

		export const child = () => {
			return tg.file("hello");
		};
	',
}
let build = job spawn {
	let job_id = job id
	let output = tg --url $local.url build --remote $path | complete
	$output | job send --tag $job_id 0
}

# The process must start while its index is held.
let output = timeout 30s tg --url $runner.url checkpoint wait runner.process.index.started $index_watch 0 | complete
success $output "the runner must start indexing the process"
let output = timeout 30s tg --url $runner.url checkpoint wait runner.process.start $start_watch 0 | complete
success $output "the process must start before its index is ready"
tg --url $runner.url checkpoint continue runner.process.start $start_watch 0
tg --url $runner.url checkpoint unwatch runner.process.start $start_watch

# The child spawn must wait rather than fail while the parent index is held.
let held = try { job recv --tag $build --timeout 5sec } catch { null }
if $held != null {
	error make { msg: "the build completed while the parent process index was held" }
}

# Release the index. The child spawn and build must complete.
tg --url $runner.url checkpoint continue runner.process.index.started $index_watch 0
tg --url $runner.url checkpoint unwatch runner.process.index.started $index_watch
let output = try { job recv --tag $build --timeout 30sec } catch { null }
if $output == null {
	error make { msg: "the build did not complete after the parent process index was released" }
}
success $output "the build must succeed after the parent process index is ready"

let file = $output.stdout | str trim
let read = tg --url $local.url get $file --depth inf | complete
success $read "the user must read the child output"
