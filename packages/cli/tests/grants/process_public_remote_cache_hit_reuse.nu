...
# A client that reuses a public process from its remote must be able to use that process's output.
# The client is allowed to reuse the process, but finishing the parent that holds its output then
# fails to authorize that output, so the two decisions disagree and a remote cache is usable only for
# a build with no parent.
#
...
# The remote holds a public finished process for the dependency.
tg --url $remote.url build --public $"($path)#dep" | ignore
