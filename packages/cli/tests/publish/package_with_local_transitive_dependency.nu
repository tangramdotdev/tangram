...
let remote_transitive_tag = run tg --url $remote.url tag get test-transitive/1.0.0 | from json | get item
...
let local_dep_obj = run tg object get $dep_id
let remote_dep_obj = run tg --url $remote.url object get $dep_id
let local_transitive_obj = run tg object get $transitive_id
let remote_transitive_obj = run tg --url $remote.url object get $transitive_id
...
let local_dep_metadata = run tg object metadata $dep_id | from json
let remote_dep_metadata = run tg --url $remote.url object metadata $dep_id | from json
let local_transitive_metadata = run tg object metadata $transitive_id | from json
let remote_transitive_metadata = run tg --url $remote.url object metadata $transitive_id | from json
...
let main_object = run tg get $main_id --blobs --depth=inf
...
let dep_object = run tg get $dep_id --blobs --depth=inf
...
let transitive_object = run tg get $transitive_id --blobs --depth=inf
