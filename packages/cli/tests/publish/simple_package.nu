...
# Publishing a simple package tags it on both the local and remote servers, syncs the object to the remote, and produces matching metadata after indexing.
...
# Verify tag on local.
let local_tag = tg tag get test-pkg/1.0.0 | from json | get target.id
assert equal $local_tag $id "Local tag does not match expected ID."

# Verify tag on remote.
let remote_tag = tg --url $remote.url tag get test-pkg/1.0.0 | from json | get target.id
