...
# Verify tag on local.
let local_tag = tg tag get test-single-file/1.0.0 | from json | get target.id
assert equal $local_tag $id "Local tag does not match expected ID."

# Verify tag on remote.
let remote_tag = tg --url $remote.url tag get test-single-file/1.0.0 | from json | get target.id
