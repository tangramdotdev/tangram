...
# Checkin the file.
let id = run tg checkin $file_path

# Publish the file.
run tg publish $file_path

# Verify tag on local.
let local_tag = run tg tag get test-single-file/1.0.0 | from json | get item
assert equal $local_tag $id "Local tag does not match expected ID."

# Verify tag on remote.
let remote_tag = run tg --url $remote.url tag get test-single-file/1.0.0 | from json | get item
