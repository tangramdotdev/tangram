...
let main_path = artifact {
	tangram.ts: '
		...
		export default function () { return `Main using: ${left()} and ${right()}`; }
...
let remote_left_tag = tg --url $remote.url tag get test-left/1.0.0 | from json | get target.id
let remote_right_tag = tg --url $remote.url tag get test-right/1.0.0 | from json | get target.id
let remote_bottom_tag = tg --url $remote.url tag get test-bottom/1.0.0 | from json | get target.id
