...
# Bob's first push transfers the directory and creates an implicit grant over the subtree he relied on.
tg --url $bob_local.url push --lazy $directory
tg --url $remote.url index

# Bob's second push skips the whole subtree through his implicit grant and transfers nothing.
