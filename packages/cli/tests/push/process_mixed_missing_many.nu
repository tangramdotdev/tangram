...
tg -u $dummy_server.url get --bytes $main_command_id | tg -u $local_server.url put --bytes -k cmd
...
while ($to_visit | length) > 0 {
	...
	let cmd_children = tg -u $dummy_server.url children $current | from json
...
tg -u $dummy_server.url get --bytes $main_output_id | tg -u $local_server.url put --bytes -k dir

# Get all file children from the output directory.
let output_files = tg -u $dummy_server.url children $main_output_id | from json
...
for i in 0..<$child_count {
	...
	let child_data = tg -u $dummy_server.url get $child_id | from json
	...
	if ($i mod 4) == 0 {
		...
		tg -u $dummy_server.url get $child_id | tg -u $remote_server.url put --id $child_id
	} else {
		# Put to local.
		tg -u $dummy_server.url get $child_id | tg -u $local_server.url put --id $child_id
	}
	...
	if ($i mod 3) == 0 {
		...
		tg -u $dummy_server.url get --bytes $child_command_id | tg -u $remote_server.url put --bytes -k cmd
		...
		tg -u $dummy_server.url get --bytes $child_output_id | tg -u $local_server.url put --bytes -k fil

		# Put the blob for this output to local.
		let child_blob_ids = tg -u $dummy_server.url children $child_output_id | from json
		for blb_id in $child_blob_ids {
			tg -u $dummy_server.url get --bytes $blb_id | tg -u $local_server.url put --bytes -k blob
		}
	} else if ($i mod 3) == 1 {
		# Put command to local, output to remote (leaf missing).
		tg -u $dummy_server.url get --bytes $child_command_id | tg -u $local_server.url put --bytes -k cmd
		for desc_id in $child_cmd_descendants {
			let kind = $desc_id | str substring 0..<3
			tg -u $dummy_server.url get --bytes $desc_id | tg -u $local_server.url put --bytes -k $kind
		}
		tg -u $dummy_server.url get --bytes $child_output_id | tg -u $remote_server.url put --bytes -k fil
...
tg -u $local_server.url push $process_id --eager --recursive --commands
