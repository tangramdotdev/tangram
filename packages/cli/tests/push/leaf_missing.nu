...
let output = tg get --bytes $dir_id | tg -u $local_server.url put --bytes  -k dir | complete
success $output

# Put the file to the local server.
let output = tg get --bytes $fil_id | tg -u $local_server.url put --bytes  -k fil | complete
success $output

# Put the blob to the remote server.
let output = tg get --bytes $blb_id | tg -u $remote_server.url put --bytes  -k blob | complete
success $output
