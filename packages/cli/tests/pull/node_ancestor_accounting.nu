...
# A named item remains pending while its ancestor requests are registered.
...
let send_watch = (
	tg --url $remote.url checkpoint watch sync.put.database.item.send --params ({
		id: $child.id,
	} | to json)
	| from json
	| get watch
...
let ancestor_watch = (
	tg --url $local.url checkpoint watch sync.get.input.item.ancestor --params ({
		id: $child.id,
	} | to json)
	| from json
	| get watch
...
tg --url $remote.url checkpoint wait sync.put.database.item.send $send_watch 0 | ignore
tg --url $local.url checkpoint wait sync.get.store.process $store_watch 0 | ignore
tg --url $remote.url checkpoint continue sync.put.database.item.send $send_watch 0
tg --url $local.url checkpoint wait sync.get.input.item.ancestor $ancestor_watch 0 | ignore
...
tg --url $local.url checkpoint continue sync.get.input.item.ancestor $ancestor_watch 0
tg --url $local.url checkpoint unwatch sync.get.input.item.ancestor $ancestor_watch
...
tg --url $remote.url checkpoint unwatch sync.put.database.item.send $send_watch
