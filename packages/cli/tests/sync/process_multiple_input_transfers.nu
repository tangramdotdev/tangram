...
# A command keeps separate sync contexts for nested inputs from two unfinished transfers at the same remote.
...
for watch in $watches {
	success (timeout 30s tg --url $remote.url --token $root_token checkpoint wait sync.control.ack $watch.ack 0 | complete) 'each input must wait on its own producer transfer'
