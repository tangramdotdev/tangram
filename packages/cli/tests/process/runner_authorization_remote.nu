...
for response in [$remote_response $local_response] {
	let body = $response.tokens.local.authorization.0 | split row '.' | get 1 | decode base64 | decode utf-8 | from json
...
let query = { 'tokens[local][authorization][0]': $remote_response.tokens.local.authorization.0 } | url build-query
...
let query = { location: remote, 'tokens[local][authorization][0]': $remote_response.tokens.local.authorization.0 } | url build-query
let denied = http get --allow-errors --full --unix-socket $runner_socket --headers $headers $'http://localhost/processes/($process)?($query)'
...
let query = { location: remote, 'tokens[local][authorization][0]': $local_response.tokens.local.authorization.0 } | url build-query
...
let targets = [
	{ location: remote, reader: $reader.token, root: $runner_root, server: $runner, token: $local_response.tokens.local.authorization.0 },
	{ location: local, reader: $remote_reader.token, root: $remote_root, server: $remote, token: $remote_response.tokens.local.authorization.0 },
]
let waits = $targets | each { |target|
	...
	let query = { lease: $spawned.lease, location: $target.location, 'tokens[local][authorization][0]': $target.token } | url build-query
...
for wait in $waits {
	...
	assert (not ($output.output.value | str contains '[authorization]')) "waiting must not mint an output capability"
