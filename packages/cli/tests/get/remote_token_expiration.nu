...
assert (($first | get --optional token) != null) "remote get should return a token"

sleep 2sec

let refreshed = tg --url $local.url get foo | from json
assert (($refreshed | get --optional token) != null) "remote get should refresh an expired token"
assert ($refreshed.token != $first.token) "the refreshed token should be new"
...
assert (($cached | get --optional token) == null) "cached get should omit an expired token"
