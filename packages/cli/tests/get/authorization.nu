...
let token = $parent.tokens.local.authorization.0 | url encode --all
let reference = $"private/^1?tokens[local][authorization][0]=($token)"
...
let reference = $"private/^1?follow=true&tokens[local][authorization][0]=($token)"
