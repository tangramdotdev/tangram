...
assert equal (tg --url $local.url tag get foo | from json | get item.id) $first

# The configured default of zero bypasses the cache.
tg --url $remote.url tag foo $second
assert equal (tg --url $local.url tag get foo | from json | get item.id) $second

# A finite override accepts the cached response.
tg --url $remote.url tag foo $third
assert equal (tg --url $local.url tag get --ttl 1h foo | from json | get item.id) $second

# An infinite override also accepts the cached response.
assert equal (tg --url $local.url tag get --no-ttl foo | from json | get item.id) $second

# An explicit zero bypasses the cache.
assert equal (tg --url $local.url tag get --ttl 0 foo | from json | get item.id) $third
