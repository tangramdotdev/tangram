...
describe("constructor", () => {
        ...
        export default tg.target(async () => {
        	const graph = await tg.graph({
        		nodes: [
        			...
        			{ kind: "file", contents: "hello from inside a graph" }
        		]
        	});
	...
	test.todo("rejects mixed graph and regular objects", async () => {
		await using server = await Server.start();
		const dir = await directory({
			"tangram.ts": `
		      export default tg.target(async () => {
	...
	test.todo("rejects multiple graph objects", async () => {
		await using server = await Server.start();
		const dir = await directory({
			"tangram.ts": `
		      export default tg.target(async () => {
