...
# Close the process's stdin and hold the close in the runner.
let close = job spawn {
	let job_id = job id
	let output = (
...
assert equal $hit.params.close "true" "the held request should be a stdin close"
...
# Hold the retention window open so that the held close is not aborted before it is serviced.
...
# Release the close, which is now serviced after the sandbox connection is gone.
