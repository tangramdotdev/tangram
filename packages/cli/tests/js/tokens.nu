...
let path = artifact {
	tangram.ts: '
		...
		export default async function () {
			...
			parent.state.tokens = { local: { authorization: ["parent-authorization", "incoming-sync"] } };
			const result = await parent.get("a");
			tg.assert(JSON.stringify(result.state.tokens.local?.authorization) === JSON.stringify(["child-authorization", "incoming-sync", "parent-authorization"]));
			tg.assert(result.state.tokens.local?.authorization?.[1] === "incoming-sync");
			const copy = result.state.tokens;
			copy.local!.authorization![1] = "changed-sync";
			tg.assert(result.state.tokens.local?.authorization?.[1] === "incoming-sync");
			const inherited = result.state.tokens;
			tg.Tokens.inherit(inherited, { remote: { authorization: ["remote-sync"] } });
			tg.assert(parent.state.tokens.remote === undefined);
			inherited.local.authorization[0] = "mutated";
			tg.assert(parent.state.tokens.local.authorization[1] === "parent-authorization");
			...
			tg.assert(decoded.options?.tokens?.local?.authorization?.includes("incoming-sync"));
			tg.assert(decoded.options?.tokens?.remote?.authorization?.[0] === "remote-sync");
			...
			first.state.tokens = { local: { authorization: ["first-authorization", "first-sync"] } };
			second.state.tokens = { local: { authorization: ["second-authorization", "second-sync"] } };
			...
			tg.assert(JSON.stringify(lifted.authorization!.toSorted()) === JSON.stringify(["first-authorization", "first-sync", "second-authorization", "second-sync"]));
			tg.assert(tg.Tokens.isEmpty(wrapper.state.tokens));
			tg.assert(tg.Tokens.isEmpty(inputs.state.tokens));
			tg.assert(first.state.tokens.local.authorization!.length === 2);
			tg.assert(second.state.tokens.local.authorization!.length === 2);
			return true;
		}
	'
}
