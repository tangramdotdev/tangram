...
let path = artifact {
	tangram.ts: '
		export default async function () {
			...
			const tokens = { local: { authorization: ["child"] } };
			...
			try {
				tg.client.getObject = async () => ({
					...
					tokens: { local: { authorization: ["parent"] } },
				});
				...
				tokens.local.authorization[0] = "mutated";
				return {
...
assert equal $output {
	children: [{ local: { authorization: [child parent] } } { local: { authorization: [child parent] } }]
	entries: [{ local: { authorization: [child parent] } } { local: { authorization: [child parent] } }]
	parent: { local: { authorization: [parent] } }
}
