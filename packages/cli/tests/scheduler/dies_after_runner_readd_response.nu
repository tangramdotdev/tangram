...
# A scheduler that dies after giving a restarted runner its new attempt, but before expiring the runner's old processes, must expire them once it restarts.
...
success (timeout 60s tg --url $local.url build --remote $trivial | complete) "the restarted runner must have received its attempt"
