...
impl Compiler {
	pub async fn declaration(
		...
		// Create the request.
		let request = super::Request::Declaration(Request {
			module: module.clone(),
			position,
		});
		...
		// Get the response.
		let super::Response::Declaration(response) = response else {
...
impl Compiler {
	pub(crate) async fn handle_declaration_request(
		&self,
		...
		// Get the locations.
		let locations = self.declaration(&module, position.into()).await?;
